document.addEventListener('DOMContentLoaded', function () {
 ...
 // Ta funkcja jest wywoływana po kliknięciu przycisku i zajmuje się przekierowaniem.
function wykonajZadanieIGoPrzekieruj(razem) {
  const bearerToken = '2612|fcmWkTUvaff9RMd2xyIXhefmDdeXFSuFKkH6eMdy'; // Twój token Bearer

  // Przygotowujesz URL API, do którego wysyłasz żądanie
  // Musisz zapewnić wartość dla zmiennej `razem`
  const url = `https://app.easycart.pl/api/dynamic-price/prod_OacwvSnmaoGtqa/${razem}`; // URL API

  const options = {
    method: 'GET',
    headers: {
      'Authorization': `Bearer ${bearerToken}`,
      // 'Content-Type' nie jest potrzebny, jeśli tylko otrzymujesz dane
    }
  };

  // Wykonujesz żądanie fetch do API
  fetch(url, options)
    .then(response => {
      if (!response.ok) {
        throw new Error('Problem z zapytaniem: ' + response.status);
      }
      return response.json(); // Oczekujesz, że odpowiedź jest w formacie JSON
    })
    .then(dataArray => {
      // Odpowiedź to prawdopodobnie tablica, więc sprawdzasz jej pierwszy element
      if (dataArray && dataArray.length > 0) {
        const redirectUrl = dataArray[0].data; // Wydobywasz URL z pierwszego elementu tablicy
        window.location.href = redirectUrl; // Przekierowujesz na ten URL
      } else {
...
// Dodajesz słuchacz zdarzeń do przycisku "dalej3"
