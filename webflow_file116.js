document.addEventListener('DOMContentLoaded', function () {
 ...
 // }
  ...
  function wykonajZadanieIGoPrzekieruj(razem) {
    const bearerToken = '2612|fcmWkTUvaff9RMd2xyIXhefmDdeXFSuFKkH6eMdy'; // Twoj token Bearer
    ...
    const url = `https://app.easycart.pl/api/dynamic-price/prod_OacwvSnmaoGtqa/${encodeURIComponent(razem)}`; // URL API, do którego wysyłasz żądanie
    ...
    fetch(url, options)
      .then(response => {
        ...
        return response.json(); // Przypuszczamy, że odpowiedź to JSON
      })
      .then(data => {
        // Załóżmy, że odpowiedź zawiera pole 'redirectUrl', które jest adresem URL do przekierowania
        const redirectUrl = data.redirectUrl; // Powinieneś dostosować klucz 'redirectUrl' do twojej odpowiedzi
  
        // Przekieruj użytkownika na otrzymany adres URL
        window.location.href = redirectUrl; // To przekieruje użytkownika
      })
      .catch(error => {
  ...
  // Dodajemy słuchacz zdarzeń do przycisku
