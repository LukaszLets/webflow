document.addEventListener('DOMContentLoaded', function () {
	...
	var button13 = document.getElementById('saveButton13');
	var button14 = document.getElementById('saveButton14');
	var dalejValue1 = '';
	var dalejValue2 = '';
	var dalejValue3 = '';
	var dalejValue4 = [];
	...
	var savetext13 = document.getElementById('savetext13');
	savetext13.style.color = 'white';
	var savetext14 = document.getElementById('savetext14');
	savetext14.style.color = 'white';
	...
	button13.addEventListener('click', function () {
		if (!button13Clicked) {
			setTextAndStyles3('13', button13, container13, savetext13);
	...
	button14.addEventListener('click', function () {
		if (!button14Clicked) {
			setTextAndStyles3('14', button14, container14, savetext14);
	...
	async function wykonajZadanieIGoPrzekieruj() {
		const bearerToken = '2612|fcmWkTUvaff9RMd2xyIXhefmDdeXFSuFKkH6eMdy';

		const url = `https://app.easycart.pl/api/dynamic-price/prod_OacwvSnmaoGtqa/${brutto2.toFixed(2)}?param1=${tekstElement2.textContent}&param2=${tekstElement3.textContent.replace("%20", "").replace("/","_")}&param3=${tekstElement4.textContent.replace("%20", "").replace("/","_")}`;
	...
	document.getElementById('dalej3').addEventListener('click', wykonajZadanieIGoPrzekieruj);
	...
	// const url = `https://app.easycart.pl/checkout/98638600/paczka-automatyzacji-do-samodzielnego-wdrozenia?plan=price_1O80ZnAiRAmm1dcU7ViSsYJO&param1=${encodeURIComponent(dalejValue1)}&param2=${encodeURIComponent(dalejValue2)}&param3=${encodeURIComponent(dalejValue3)}&param4=${encodeURIComponent(param4)}`;
	...
	function setTextAndStyles(value, clickedButton, clickedContainer, text) {
		...
		let allButtons = [button1, button2];
		...
		let allTxts2 = [savetext1, savetext2];
		allTxts.forEach(txts => {
			if (txts == text) {
	...
	function setTextAndStyles3(value, clickedButton, clickedContainer, text) {
		...
		if (value === '6') {
			...
			textElement3.textContent += 'Discord';
			savetext13.style.color = 'black';
			savetext13.innerHTML = 'Wybrano';
		} else if (value === '14') {
			textElement3.textContent += 'Telegram';
			savetext14.style.color = 'black';
			savetext14.innerHTML = 'Wybrano';
		}
	...
	function resetButton13() {
		button13.style.backgroundColor = '#1E2024';
		button13.style.color = '#FFFFFF';
		container13.style.border = '1px solid transparent';
		savetext13.style.color = 'white';
		savetext13.innerHTML = 'Wybieram';
		var values = textElement3.textContent.split('/').filter(v => v.trim() !== 'Discord');
		textElement3.textContent = values.join(' / ');
		tekstElement4.textContent = values.join(' / ');
		dalejValue3 = values.join(' / ');
		if (dalejValue2 === '' || dalejValue3 === '') {
	...
	function resetButton14() {
		button14.style.backgroundColor = '#1E2024';
		button14.style.color = '#FFFFFF';
		container14.style.border = '1px solid transparent';
		savetext14.style.color = 'white';
		savetext14.innerHTML = 'Wybieram';
		var values = textElement3.textContent.split('/').filter(v => v.trim() !== 'Telegram');
		textElement3.textContent = values.join(' / ');
		tekstElement4.textContent = values.join(' / ');
		dalejValue3 = values.join(' / ');
		if (dalejValue2 === '' || dalejValue3 === '') {
	...
	function resetButton9() {
		...
		dalejValue4 = dalejValue4.filter(item => item.nazwa !== 'Konsultacja lub pomoc we wdrożeniu');
	...
	function resetButton10() {
		...
		dalejValue4 = dalejValue4.filter(item => item.nazwa !== 'Ponowne przypomnienie dla handlowca');
		if (dalejValue4.length === 0) {
			nie_wybrano.style.display = 'block';
		}
		// var values = textElement3.textContent
	...
	function resetButton11() {
		...
		dalejValue4 = dalejValue4.filter(item => item.nazwa !== 'Automatyczny dobór handlowca');
		if (dalejValue4.length === 0) {
			nie_wybrano.style.display = 'block';
		}
		// var values = textElement3.textContent
	...
	function resetButton12() {
		...
		dalejValue4 = dalejValue4.filter(item => item.nazwa !== "Zbieranie lead'ów z różnych źródeł");
		if (dalejValue4.length === 0) {
			nie_wybrano.style.display = 'block';
		}
		// var values = textElement3.textContent
	...
	function resetAllStylesForButtons678() {
		let buttons = [button6, button7, button8, button13, button14];
		let containers = [container6, container7, container8, container13, container14];
		buttons.forEach(btn => {
			btn.style.backgroundColor = '#1E2024';
			btn.style.color = '#FFFFFF';
		});
